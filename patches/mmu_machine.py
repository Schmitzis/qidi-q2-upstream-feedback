...
class MmuToolHead(toolhead.ToolHead, object):
    ...
    def handle_connect(self):
        ...
        if self.mmu_machine.homing_extruder:
            ...
            # Kalico refactored PrinterExtruder to a real extruder_steppers list
            # (read by e.g. cmd_default_SET_PRESSURE_ADVANCE) instead of a single
            # `extruder_stepper` attribute. A direct assignment here leaves that
            # list empty -> "Active extruder does not have a stepper". Register
            # through the real API so both the list and the back-compat alias
            # (see config-patches/klippy/extruder.py) get set correctly.
...
class MmuExtruderStepper(ExtruderStepper, object):
    ...
    # Original override called self.pressure_advance/self._set_pressure_advance,
    # which only existed in pre-refactor Klipper. Kalico's ExtruderStepper (this
    # class's real parent, imported from kinematics.extruder) replaced those with
    # a pa_model/smoother-based system exposing no equivalent attributes, so this
    # override just crashed ("no attribute 'pressure_advance'") instead of setting
    # anything. Removed - falls back to the parent's real, working implementation.
